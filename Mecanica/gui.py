...
def _boton(parent, text, command, color=None, hover=None, width=None, **kw):
    ...
    # En macOS, tk.Button puede ignorar bg/fg por el tema Aqua.
    # Usamos tk.Label como botón visual solo en Mac.
    ...
    # Windows/Linux: se conserva tu botón original
    btn = tk.Button(
        parent,
        text=text,
        command=command,
...
class BarraSuperior(tk.Frame):
    ...
    def _construir(self):
        ...
        if sys.platform == "darwin":
            ...
            self.btn_1d.bind("<Button-1>", lambda e: self.ctrl.cambiar_dimension(1))
            self.btn_1d.pack(side=tk.LEFT)

            self.btn_2d = tk.Label(
                fd,
                text="2D",
                bg=PALETA["acento"],
                fg="#ffffff",
            ...
            self.btn_2d.bind("<Button-1>", lambda e: self.ctrl.cambiar_dimension(2))
...
class VentanaPrincipal(tk.Tk):
    ...
    on_point_drag_released=self._tras_drag_punto
)
    ...
    def _tras_drag_punto(self, id_punto: int):
        """
        Llamado cuando se suelta un punto de prueba arrastrado.
        Actualiza la lista de puntos y recalcula el campo eléctrico.
        """
        sistema = self.estado.sistema_activo
        dim = self.estado.dimension_activa

        # Actualizar lista de puntos con la nueva posición
        self.panel_ctrl.actualizar_lista_puntos(sistema.puntos_prueba, dim)

        # Recalcular campos si hay cargas y puntos
        if sistema.cargas and sistema.puntos_prueba:
            try:
                campos = sistema.calcular_todos_los_campos()
                ...
                self.barra_estado.exito(
                    f"Punto [id={id_punto}] movido — campo eléctrico actualizado."
                )
            ...
            self.canvas_vis.actualizar()
            self.barra_estado.exito(f"Punto [id={id_punto}] movido.")
